...
if [ $genome == "hg19" ];then
	for chr in chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22 chrX chrY;do
		$path/compartment_calling.pl $myPath/hg19.TSS.${bin}.file $outpath/$chr.matrix.component > $outpath/$chr.bin.$bin.chip.freq.score
	done
fi

if [ $genome == "hg38" ];then
        for chr in chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chr20 chr21 chr22 chrX chrY;do
                $path/compartment_calling.pl $myPath/hg38.TSS.${bin}.file $outpath/$chr.matrix.component > $outpath/$chr.bin.$bin.chip.freq.score
        done
fi
...
if [ $genome == "mm10" ];then
        for chr in chr1 chr2 chr3 chr4 chr5 chr6 chr7 chr8 chr9 chr10 chr11 chr12 chr13 chr14 chr15 chr16 chr17 chr18 chr19 chrX chrY;do
                $path/compartment_calling.pl $myPath/mm10.TSS.${bin}.file $outpath/$chr.matrix.component > $outpath/$chr.bin.$bin.chip.freq.score
        done
fi
