...
open(OUT_filtered_inward_samestrand, ">filtered_inward_samestrand.txt");
open(OUT_filtered_inward_acs,">filtered_inward_acs.txt");
...
my $total_inward = 0;
my $total_outward = 0;
my $across_cutting_site=0;
my $count_samestrand = 0;
my $count_inward = 0;
my $count_outward = 0;
my $total_same_fragment=0;
...
while(my $line = <FH>){
	...
	if($chr1 ne $chr2){
		...
		my ($frag1, $left1, $right1) = find_frag($frag_HASH->{$chr1}, $frag_loc, $beg1, $str1, $len);
		my ($frag2, $left2, $right2) = find_frag($frag_HASH->{$chr2}, $frag_loc, $beg2, $str2, $len);

		if((!$frag1) || (!$frag2)){
			$across_cutting_site++;
			if(!($str1 eq $str2) && (($str1 eq "+" && $beg1 < $beg2) || ($str2 eq "+" && $beg2 < $beg1)) ){
				print OUT_filtered_inward_acs join("\t", $frag1, $frag2, $chr1, $beg1, $left1, $right1, $str1, $chr2, $beg2, $left2, $right2, $str2)."\n";
              		}
			next;
		}
		...
		if($frag1 eq $frag2){
			$total_same_fragment ++;
			if(!($str1 eq $str2) && (($str1 eq "+" && $beg1 < $beg2) || ($str2 eq "+" && $beg2 < $beg1)) ){
                                print OUT_filtered_inward_samestrand join("\t", $frag1, $frag2,$chr1, $beg1, $str1, $chr2, $beg2, $str2)."\n";
                        }
...
print SUMM join("\t", $expt, $total_inter/2, $total_intra/2, $total_samestrand/2, $total_inward/2, $total_outward/2, $count_samestrand/2, $count_inward/2, $count_outward/2, $total_same_fragment/2, $across_cutting_site/2)."\n";
close(SUMM);
