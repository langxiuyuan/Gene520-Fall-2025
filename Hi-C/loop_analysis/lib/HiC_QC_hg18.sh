...
hg18=/mnt/NFS/homeGene/JinLab/fxj45/Illumina_iGenomes/Homo_sapiens/UCSC/hg18/Sequence/BowtieIndex/genome # bowtie index for hg18
lib=~/lib/HiC
...
then
	rdlen=36
	let trlen=$lenth-36		#trim length. $2 need to be the length of sequence reads
	# bowtie mapping, ONE CPU each process
	$bowtie -v 3 -m 1 --trim3 $trlen --best --strata --time -p 1 --sam $hg18 $expt.R1.fastq $expt.R1.sam &
	$bowtie -v 3 -m 1 --trim3 $trlen --best --strata --time -p 1 --sam $hg18 $expt.R2.fastq $expt.R2.sam &
else
	rdlen=$length
	$bowtie -v 3 -m 1 --best --strata --time -p 1 --sam $hg18 $expt.R1.fastq $expt.R1.sam &
        $bowtie -v 3 -m 1 --best --strata --time -p 1 --sam $hg18 $expt.R2.fastq $expt.R2.sam &
fi
# wait till finish
wait

# SAM files into paired-end BAM files
hg18=/mnt/NFS/homeGene/JinLab/fxj45/GenebankDB/hg18/hg18.fa.fai
...
$lib/pairing_two_SAM_reads.pl $expt.R1.sam $expt.R2.sam | samtools view -bS -t $hg18 -o - - > $expt.bam &
# wait till finish
wait
...
samtools sort -o $expt.bam $expt.sorted | samtools view - | ~/lib/HiC/remove_dup_PE_SAM_sorted.pl | samtools view -bS -t $hg18 -o - - > $expt.sorted.nodup.bam
...
$lib/reads_2_cis_frag_loop.pl ~/lib/ELPU/hg18.HindIII.frag.bed $rdlen $expt.loop.inward $expt.loop.outward $expt.loop.samestrand summary.frag_loop.read_count $expt $expt.temp &
